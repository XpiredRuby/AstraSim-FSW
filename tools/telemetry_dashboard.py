...
def main() -> None:
    ...
    while True:
        ...
        print("AstraSim-FSW Live Dashboard")
        print("=" * 32)
        print(f"Source: {address[0]}:{address[1]}")
        print(f"Seq:    {packet['sequence_number']}")
        print(f"Mode:   {packet['mode']}")
        print(f"Fault:  {packet['last_fault']}")
        print(f"CPU:    {packet['cpu_load_percent']:.2f}%")
        print(f"Mem:    {packet['memory_load_percent']:.2f}%")
        print(f"HB:     {packet['heartbeat_count']}")
        print("-" * 32)
        print(f"Ack seq:    {packet['last_command_sequence_number']}")
        print(f"Ack command:{packet['last_command_id']}")
        print(f"Ack status: {packet['last_command_status']}")
